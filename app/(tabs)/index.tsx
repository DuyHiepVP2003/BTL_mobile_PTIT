...
export default function HomeScreen() {
  ...
  const fetchWeatherData = async (locationQuery: string) => {
    try {
      ...
      const windChange = (
        data.current.wind_kph - data.forecast.forecastday[0].hour[0].wind_kph
      ).toFixed(1)
      const pressureChange = (
        data.current.pressure_mb -
        data.forecast.forecastday[0].hour[0].pressure_mb
      ).toFixed(0)
      const rainChange = (
      ...
      const formattedData: WeatherData = {
        ...
        dayTemp: `Ngày ${Math.round(
          data.forecast.forecastday[0].day.maxtemp_c
        )}°`,
        nightTemp: `Đêm ${Math.round(
